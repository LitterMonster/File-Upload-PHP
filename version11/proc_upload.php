...
for ($i = 0; $i < $count; $i++) 
{
    ...
    {
        echo "<script>alert('".$true_name." has existed in ".
            $des_dir."')</script>";
        continue;
    }
    ...
    if($_FILES["userfiles"]["error"][$i] > 0)
    {
        ...
        echo "<script>alert('Something Wrong:Error code:".$wrong_id."')</script>";
...
if (!empty($success) && empty($failure))
    echo "<script>alert('Success:".$succnum."个;;; Fail:0个;;;".
    $success."Delete fileSuccess!"."')</script>";
else if (!empty($success) && !empty($failure))
    echo "<script>alert('Success:".$succnum."个;;; Fail:$failnum个;;;".
    $success."Delete fileSuccess!".$failure."Delete file Fail!')</script>";
else if (empty($success) && !empty($failure))
    echo "<script>alert('Success:0;;; Fail".$failnum.";;;".
    $failure."Delete file Fail!')</script>";
echo "<script>window.location.href='upload_file.php';</script>";
?>
